...
export default function AdminUsersPage() {
  ...
  const inputClass = "w-full px-3 py-2 rounded-lg bg-white dark:bg-emerald-900/40 border border-emerald-200 dark:border-emerald-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none text-emerald-900 dark:text-white placeholder-emerald-400 text-sm";

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-emerald-900 dark:text-white">Users</h2>
      ...
      {editUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setEditUser(null)}>
          <div className="bg-white dark:bg-emerald-900 rounded-2xl border border-emerald-100 dark:border-emerald-700 p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-bold text-emerald-900 dark:text-white mb-4">Edit User — {editUser}</h3>
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm text-emerald-800 dark:text-emerald-200">
                <input type="checkbox" checked={editForm.is_verified} onChange={(e) => setEditForm((p) => ({ ...p, is_verified: e.target.checked }))} className="rounded border-emerald-200 dark:border-emerald-700 bg-white dark:bg-emerald-900/40 text-emerald-500 focus:ring-emerald-500" />
                Verified
              </label>
              <div className="flex gap-2 pt-2">
                ...
                <button type="button" onClick={() => setEditUser(null)} className="px-4 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-800 text-emerald-800 dark:text-emerald-200 text-sm hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer">Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="overflow-x-auto rounded-xl bg-white dark:bg-emerald-900 border border-emerald-100 dark:border-emerald-700">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? [1,2,3,4,5].map(i => <SkeletonRow key={i} cols={7} />) : users.length === 0 ? <tr><td colSpan={7} className="p-6 text-center text-emerald-500 dark:text-emerald-500">No users found</td></tr> : users.map((u) => (
              <tr key={u.user_id} className="border-b border-emerald-100 dark:border-emerald-700 text-emerald-800 dark:text-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-800/50">
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(u)} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-blue-400 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Edit2 size={12} /></button>
                    <button onClick={() => handleDelete(u.user_id)} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-red-400 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Trash2 size={12} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {users.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-emerald-500 dark:text-emerald-500">No users found</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
