...
export default function AdminPlansPage() {
  ...
  const inputClass = "w-full px-3 py-2 rounded-lg bg-white dark:bg-emerald-900/40 border border-emerald-200 dark:border-emerald-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none text-emerald-900 dark:text-emerald-100 placeholder-emerald-400 text-sm";

  return (
    <div className="space-y-4">
      {/* Tabs */}
      <div className="flex items-center justify-between">
        <div className="flex gap-1 p-1 rounded-xl bg-emerald-100 dark:bg-emerald-900">
          <button onClick={() => setTab('plans')} className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${tab === 'plans' ? 'bg-white dark:bg-emerald-800 text-emerald-900 dark:text-white shadow-sm' : 'text-emerald-600 dark:text-emerald-400 hover:text-emerald-800'}`}>Plans</button>
          <button onClick={() => setTab('users')} className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${tab === 'users' ? 'bg-white dark:bg-emerald-800 text-emerald-900 dark:text-white shadow-sm' : 'text-emerald-600 dark:text-emerald-400 hover:text-emerald-800'}`}>Users</button>
        </div>
        {tab === 'plans' && (
      ...
      {success && <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800"><CheckCircle size={14} className="text-emerald-600 dark:text-emerald-400" /><p className="text-xs text-emerald-700 dark:text-emerald-300">{success}</p></div>}
      ...
      {(showAdd || edit) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={resetForm}>
          <div className="bg-white dark:bg-emerald-900 rounded-2xl border border-emerald-100 dark:border-emerald-700 p-6 w-full max-w-md mx-4 shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-emerald-900 dark:text-white">{edit ? 'Edit Plan' : 'Add Plan'}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="block text-xs text-emerald-700 dark:text-emerald-300 mb-1">Plan Name</label>
                <input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. PREMIUM" className={inputClass} />
              </div>
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-xs text-emerald-700 dark:text-emerald-300 mb-1">Price (₹)</label>
                  <input type="number" min="0" value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: parseInt(e.target.value) || 0 }))} className={inputClass} />
                </div>
                <div className="flex-1">
                  <label className="block text-xs text-emerald-700 dark:text-emerald-300 mb-1">Coins</label>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {plans.length === 0 ? (
                ...
                <tr key={p.id} className="border-b border-emerald-100 dark:border-emerald-700 text-emerald-800 dark:text-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-800/50">
                  <td className="p-3">{p.id}</td>
                  <td className="p-3 font-medium text-emerald-900 dark:text-white">{p.name}</td>
                  <td className="p-3">₹{p.price}</td>
                  <td className="p-3">{p.coins}</td>
                  <td className="p-3">{p.price > 0 ? (p.coins / p.price).toFixed(1) + 'x' : '-'}</td>
                  <td className="p-3">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(p)} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-blue-600 dark:text-blue-400 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Edit2 size={12} /></button>
                      <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-red-500 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
