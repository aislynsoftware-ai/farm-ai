...
export default function AdminCropsPage() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-gray-900 dark:text-white">Crops</h2>
      ...
      <form onSubmit={handleSave} className="rounded-xl bg-white dark:bg-gray-800 border border-emerald-200 dark:border-emerald-700 p-4 flex flex-wrap items-end gap-3">
        ...
        <div>
          <label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Agri Title</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {loading ? [1,2,3,4].map(i => <SkeletonCard key={i} />) : items.map((c) => (
          <div key={c.id} className="rounded-xl bg-white dark:bg-gray-800 border border-emerald-200 dark:border-emerald-700 overflow-hidden group">
            ...
            <div className="p-3">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{c.agri_title || ''}</p>
              <div className="flex gap-1 mt-2">
                <button onClick={() => { setEdit(c); setTitle(c.title); setAgriId(c.agri_id || ''); setImage(null); setPreview(c.image_url || ''); }} className="p-1.5 rounded bg-gray-100 dark:bg-gray-700 text-blue-400 hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer"><Edit2 size={12} /></button>
                <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded bg-gray-100 dark:bg-gray-700 text-red-400 hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer"><Trash2 size={12} /></button>
              </div>
            </div>
          </div>
        ))}
