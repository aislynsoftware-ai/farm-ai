...
export default function AdminProductsPage() {
  ...
  const Modal = ({ title, onSubmit }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 overflow-y-auto py-8" onClick={closeModal}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 w-full max-w-2xl mx-4 my-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          {!editModal && (
            <div>
              ...
              <select value={form.crop_sub_id} onChange={(e) => setForm((p) => ({ ...p, crop_sub_id: e.target.value }))} className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs">
          ...
          <div>
            ...
            <input value={form.disease_name} onChange={(e) => setForm((p) => ({ ...p, disease_name: e.target.value }))} className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" />
          </div>
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input value={form[`product${i}_name`]} onChange={(e) => setForm((p) => ({ ...p, [`product${i}_name`]: e.target.value }))} placeholder="Name" className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" />
                <input value={form[`product${i}_url`]} onChange={(e) => setForm((p) => ({ ...p, [`product${i}_url`]: e.target.value }))} placeholder="URL" className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" />
          ...
          <div className="grid grid-cols-3 gap-2">
            <div><label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Fertilizers</label><input value={form.fertilizers} onChange={(e) => setForm((p) => ({ ...p, fertilizers: e.target.value }))} className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" /></div>
            <div><label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Pesticides</label><input value={form.pesticides} onChange={(e) => setForm((p) => ({ ...p, pesticides: e.target.value }))} className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" /></div>
            <div><label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Care Points</label><input value={form.care_points} onChange={(e) => setForm((p) => ({ ...p, care_points: e.target.value }))} className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 focus:border-emerald-500 outline-none text-gray-900 dark:text-white text-xs" /></div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? [1,2,3].map(i => <Skeleton key={i} className="h-44" />) : items.map((crop) => (
        <div key={crop.id} className="rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          {crop.products && crop.products.length > 0 ? crop.products.map((disease, di) => (
            <div key={di} className="border-b border-gray-200 dark:border-gray-700/50 last:border-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 p-4">
                {disease.products && disease.products.filter((p) => p.product_name).map((prod, pi) => (
                  <div key={pi} className="rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 overflow-hidden">
                    {prod.product_image ? (
      ...
      {!loading && items.length === 0 && <div className="p-6 text-center text-emerald-500 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">No products found</div>}
    </div>
  );
}
