...
export default function AdminSubCropsPage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setEdit(null); setTitle(''); setCropId(''); setImage(null);
      load();
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSave} className="rounded-xl bg-gray-800/80 border border-gray-700 p-4 flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <input type="file" accept="image/*" className="hidden" ref={fileRef} onChange={(e) => setImage(e.target.files?.[0] || null)} />
        ...
        {edit && <button type="button" onClick={() => { setEdit(null); setTitle(''); setCropId(''); setImage(null); }} className="px-3 py-2 rounded-lg bg-gray-700 text-gray-300 text-sm hover:bg-gray-600 cursor-pointer">Cancel</button>}
      </form>
