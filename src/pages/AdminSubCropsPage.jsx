...
export default function AdminSubCropsPage() {
  ...
  const inputClass = "w-full px-3 py-2 rounded-lg bg-white dark:bg-emerald-900/40 border border-emerald-200 dark:border-emerald-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none text-emerald-900 dark:text-white placeholder-emerald-500 dark:placeholder-emerald-400 text-sm";

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-emerald-900 dark:text-white">Sub Crops</h2>
      ...
      <form onSubmit={handleSave} className="rounded-xl bg-white dark:bg-emerald-900 border border-emerald-100 dark:border-emerald-700 p-4 flex flex-wrap items-end gap-3">
        ...
        <div>
          <label className="block text-xs text-emerald-600 dark:text-emerald-400 mb-1">Crop</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {loading ? [1,2,3,4].map(i => <SkeletonCard key={i} />) : items.map((s) => (
          <div key={s.id} className="rounded-xl bg-white dark:bg-emerald-900 border border-emerald-100 dark:border-emerald-700 overflow-hidden group">
            {s.image_url && <img src={s.image_url} alt="" className="w-full h-28 object-cover" />}
            <div className="p-3">
              <p className="text-sm font-medium text-emerald-900 dark:text-white truncate">{s.title}</p>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 truncate">{s.crop_name || ''}</p>
              <div className="flex gap-1 mt-2">
                <button onClick={() => { setEdit(s); setTitle(s.title); setCropId(s.crop_id || ''); setImage(null); setPreview(s.image_url || ''); }} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-blue-400 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Edit2 size={12} /></button>
                <button onClick={() => handleDelete(s.id)} className="p-1.5 rounded bg-emerald-50 dark:bg-emerald-800 text-red-400 hover:bg-emerald-100 dark:hover:bg-emerald-700 cursor-pointer"><Trash2 size={12} /></button>
              </div>
            </div>
          </div>
        ))}
